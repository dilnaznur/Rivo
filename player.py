...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [3, 40, 2]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        """
        data = [0] * 3
        for temp, obstacle in enumerate(obstacles):
            if temp < 2:
                data[temp] = obstacle['y']
        if obstacles:
            if player_x > obstacles[0]['x']:
                data[2] = player_x - obstacles[0]['x']
            else:
                data[2] = obstacles[0]['x'] - player_x
        else:
            data[2] = player_x
        nn_output = self.nn.forward(np.array(data).reshape(-1, 1))
